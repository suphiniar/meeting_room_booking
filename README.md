...
1. 会议室管理：新增、删除会议室
2. 用户管理：
    - 提升/降级用户角色（普通用户 ↔ manager，不能修改自己权限）
    - 删除用户账号（不能删除自己）
    - 修改密码（两种方式）
        - 自定义新密码
        - 一键重置密码为 123456
...
### 管理用户（manager角色）
1. 可提交预约，查看全部预约记录
2. 预约取消权限：**仅可以取消自己创建的预约 或 user普通用户创建的预约，不能删除其他manager、admin的预约**
3. 首页与全部预约页面会根据角色动态渲染取消按钮，无权限时显示“无权取消”
...
3. 权限控制：区分 admin / manager / user 三种角色
    - admin：全权限
    - manager：预约查看权限，预约删除有范围限制
    - user：仅操作自己的预约
...
4. 时间校验：不允许预约过去的时间
5. manager角色预约删除：无法删除admin、其他manager创建的预约，前后端双重校验

## 备注
1. 点击这个黄色按钮，就能**把你的 Flask 会议室预约网站再续一个月**，网站（用户名：3038913903）不会下线；
